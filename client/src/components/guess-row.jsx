...
const GuessRow = (props) => {
    ...
    return (
        <div id={`row-${props.attempt}`} className="guess-row">
            ...
            <div id={`error-hard-row-${props.attempt}`} className={"error-hard hide"}>{props.error ? `${props.error.index}${ordinals[props.error.index]} letter must be ${props.error.letter}` : ""}</div>
        </div>
    );
}
